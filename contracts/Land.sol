...
contract Land {
    ...
    function getRequestedLandsCount() public view returns (uint256) {
        uint256 count = 0;
        for (uint256 i = 0; i < 1000; i++) {
            // assuming the mapping keys are integers between 0 and 999
            if (RequestedLands[i]) {
                count++;
            }
        }
    ...
    function LandOwnershipTransfer(uint256 _landId, address _newOwner) public {
        require(isLandInspector(msg.sender));
        delete RequestsMapping[_landId];
        delete RequestedLands[_landId];
